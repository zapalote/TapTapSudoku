...
class Main extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      appStateListener: AppState.addEventListener('change', this.handleAppStateChange)
    });
    // Listen to orientation changes
    this.setState({
      orientationListener: Dimensions.addEventListener('change', this.handleOrientationChange)
    });
  ...
  componentWillUnmount() {
    this.state.appStateListener && this.state.appStateListener.remove();
    this.state.orientationListener && this.state.orientationListener.remove();
  }
  ...
  handleAppStateChange = async (nextAppState) => {
    ...
    if (this.state.appState.match(/inactive|background/) && nextAppState === 'active' ){
      const fromStore = await this.loadBoardFromStore();
      if (!fromStore) await this.newGame();
    } else {
  ...
  saveBoardToStore = async () => {
    await Store.set('error', this.error);
    await Store.set('records', this.records);
    await Store.set('board', this.state.game);
    await Store.set('elapsed', this.timer.getElapsed());
    await Store.set('levelRange', this.state.levelRange);
    await Store.set('levelValue', this.state.levelValue);
  ...
  loadBoardFromStore = async () => {
    ...
    if(game ===  null || this.state.storeError) {
      // no game in store or error
      this.setState({
        showMenu: true,
        loading: false,
        game: null,
  ...
  showInfo = () => {
    ...
    const info = ''; // nok `first: ${this.firstTime} StoreErr: ${this.state.storeError}\n`;
